        ...
        {connected && user ? (
          <div className="flex items-center gap-3 bg-white/[0.02] border border-white/[0.06] rounded-xl pl-3.5 pr-2 py-1.5 animate-fade-in relative group">
            <div className="text-left select-none max-w-[120px] md:inline hidden">
              ...
              <div className="text-[9px] font-mono text-slate-500 truncate mt-0.5" title={user.address}>
                {user.hideWallet ? 'Wallet Hidden' : (user.address.length > 12 ? user.address.slice(0, 6) + '...' + user.address.slice(-4) : user.address)}
              </div>
            </div>
