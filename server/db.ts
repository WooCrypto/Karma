...
// --- Dynamic Out-Of-Browser Sync Sessions ---
...
export async function createSyncSession(id: string, session: SyncSession): Promise<void> {
  const pathStr = `sync_sessions/${id}`;
  try {
    await setDoc(doc(db, 'sync_sessions', id), session);
  } catch (err) {
    handleFirestoreError(err, OperationType.WRITE, pathStr);
  }
}

export async function getSyncSession(id: string): Promise<SyncSession | null> {
  const pathStr = `sync_sessions/${id}`;
  try {
    const snap = await getDoc(doc(db, 'sync_sessions', id));
    if (!snap.exists()) return null;
    return snap.data() as SyncSession;
  } catch (err) {
    handleFirestoreError(err, OperationType.GET, pathStr);
    return null;
  }
...
export async function updateSyncSession(id: string, sessionUpdates: Partial<SyncSession>): Promise<void> {
  const pathStr = `sync_sessions/${id}`;
  try {
    const current = await getSyncSession(id);
    if (!current) throw new Error(`Sync session not found for id: ${id}`);
    await setDoc(doc(db, 'sync_sessions', id), { ...current, ...sessionUpdates });
  } catch (err) {
    handleFirestoreError(err, OperationType.WRITE, pathStr);
  }
}
...
export async function deleteSyncSession(id: string): Promise<void> {
  const pathStr = `sync_sessions/${id}`;
  try {
    await deleteDoc(doc(db, 'sync_sessions', id));
  } catch (err) {
    handleFirestoreError(err, OperationType.DELETE, pathStr);
  }
}
